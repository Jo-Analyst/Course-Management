            ...
            try
            {
                using (var connection = new SqlConnection(DbConnectionString.connectionString))
                {
                    ...
                    command.CommandText = "" +
                        ...
                        "CREATE TABLE [dbo].[Students] (    [Id]         INT           IDENTITY (1, 1) NOT NULL,    [name]       VARCHAR (100) NOT NULL,    [CPF] VARCHAR(MAX) NULL,   [gender]     VARCHAR (1)   NOT NULL, [level] VARCHAR(5) NULL,    [class_id]   INT           NULL,    [created_at] DATETIME      NOT NULL,    [updated_at] DATETIME      NULL,    PRIMARY KEY CLUSTERED ([Id] ASC),    FOREIGN KEY ([class_id]) REFERENCES [dbo].[Classes] ([Id]) ON DELETE SET NULL);" +
